...
def find_extrema_custom(sig, fs, f_range, boundary=0, first_extrema='peak',
    ...
    if pad and f_range is not None:
    ...
    if f_range is not None:
        sig = filter_signal(sig, fs, pass_type, f_range, remove_edges=False, **filter_kwargs)

    # Find rising and decaying zero-crossings
    rise_xs = find_flank_zerox(sig, 'rise')
    decay_xs = find_flank_zerox(sig, 'decay')
